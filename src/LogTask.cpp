    ...
    : prefixedName(prefix + taskName)
{
    try
    {
        ...
        BOOST_LOG_TRIVIAL(info) << "created task " << taskName;
    }
    catch(std::runtime_error& e)
    {
        BOOST_LOG_TRIVIAL(warning) << "could not create task " << taskName << " due to missing typekit";
    }
}
...
std::unique_ptr<LogTask::PortHandle> LogTask::createPortHandle(const std::string& portName, pocolog_cpp::InputDataStream& inputStream)
{
    ...
    if(!type)
    {
        BOOST_LOG_TRIVIAL(warning) << "cannot find " << inputStream.getCXXType() << " in the type info repository";
        return nullptr;
    }
    ...
    if(!typekitTransport)
    {
        BOOST_LOG_TRIVIAL(warning) << "cannot report ports of type " << type->getTypeName() << " as no typekit generated by orogen defines it";
        return nullptr;
    }
...
bool LogTask::unmarshalSample(std::unique_ptr<PortHandle>& portHandle, uint64_t indexInStream)
{
    ...
    {
        BOOST_LOG_TRIVIAL(warning) << "Warning, could not replay sample: " << portHandle->inputDataStream.getName() << " " << indexInStream;
        return false;
    }
    ...
    {
        BOOST_LOG_TRIVIAL(error) << "caught marshall error...";
        return false;
    }
